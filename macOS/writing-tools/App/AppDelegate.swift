...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    // NEW: Properties for the Local LLM Store window.
    private var llmStoreWindow: NSWindow?
    private var llmStoreHostingView: NSHostingView<LocalLLMStoreView>?
    ...
    private func setupMenuBar() {
        ...
        // NEW: Add menu item to open the Local LLM Store window.
        menu.addItem(NSMenuItem(title: "Local LLM Store", action: #selector(showLocalLLMStore), keyEquivalent: "l"))
    ...
    // NEW: Show Local LLM Store window.
    @objc private func showLocalLLMStore() {
        // Close any existing LLM Store window.
        llmStoreWindow?.close()
        llmStoreWindow = nil
        llmStoreHostingView = nil
        
        llmStoreWindow = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 800, height: 600),
            styleMask: [.titled, .closable, .resizable],
            backing: .buffered,
            defer: false
        )
        llmStoreWindow?.title = "Local LLM Model Store"
        llmStoreWindow?.center()
        llmStoreWindow?.isReleasedWhenClosed = false
        
        let storeView = LocalLLMStoreView()
        llmStoreHostingView = NSHostingView(rootView: storeView)
        llmStoreWindow?.contentView = llmStoreHostingView
        llmStoreWindow?.delegate = self
        
        NSApp.activate(ignoringOtherApps: true)
        llmStoreWindow?.makeKeyAndOrderFront(nil)
    }
    ...
    @objc func showLocalLLMStoreManually() {
        // Close any existing store window.
        llmStoreWindow?.close()
        llmStoreWindow = nil
        llmStoreHostingView = nil
